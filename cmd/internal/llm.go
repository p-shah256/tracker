...
var clean = NewCleaner()

func ParseJobDesc(htmlFilePath string) (map[string]interface{}, error) {
	parsingRulesPath := filepath.Join(".", "src", "config", "1_parsing.txt")
...
func GetTailored(dbFriendly map[string]interface{}, relevantYAML map[string]interface{}) (string, error) {
	tailorPath := filepath.Join(".", "src", "config", "3_tailor.txt")
	...
	userMessage := fmt.Sprintf(
		...
		string(dbFriendlyJSON), string(relevantYAML))
